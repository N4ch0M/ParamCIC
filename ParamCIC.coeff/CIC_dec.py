"""Parameterized CIC Decimator Filter using Python"""

# Bloque 1 Genera el filtro CIC Decimador
# ============================================
...
R = 2               # Factor de decimación
M = 2               # Retardo diferencial
N = 3               # Número de secciones

# Función de transferencia en frecuencia
z = np.exp(2j * np.pi * np.linspace(0, 0.5, 1024))  # Sin multiplicar por R

# Respuesta en frecuencia del filtro CIC (en términos de Z)
numerator = (1 - z**(-M))**N
denominator = (1 - z**(-1))**N
# Evita división por cero en el denominador
...
magnitude = np.abs(h)
# Evita valores extremadamente pequeños para evitar problemas numéricos
magnitude = np.maximum(magnitude, 1e-9)

# Convertimos la frecuencia normalizada a Hz, ajustando por el factor de decimación
f = np.linspace(0, f_clk / R / 2, 1024)  # Se divide f_clk por R
...
# Magnitud en dB
plt.plot(f, 20 * np.log10(magnitude), label='CIC Decimator')

# Personaliza la gráfica
plt.title('Respuesta en Frecuencia del CIC Decimador')
